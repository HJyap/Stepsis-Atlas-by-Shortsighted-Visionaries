...
def extract_article_data(text_content: str) -> dict:
    ...
    # Initialize the LLM using OpenRouter to access Claude
    llm = ChatOpenAI(
        base_url="https://openrouter.ai/api/v1",
        api_key=API_KEY,
        model="anthropic/claude-sonnet-4.6", # Your requested model
        temperature=0.0
    )
...
def process_chunks_to_relational_db(input_mapping_file, chunks_dir, output_dir):
    ...
    for source_json, required_chunk_ids in articles_map.items():
        ...
        print(f"🧠 Extracting relational data for {source_json} using Claude Sonnet 4.6...")
        
        try:
            extracted_json = extract_article_data(combined_text)
            
            # Save the resulting JSON file
            output_filename = source_json.replace("_chunks.json", ".json").replace(".json", "_relational.json")
...
if __name__ == "__main__":
    ...
    CHUNKS_DIRECTORY = "/Users/hongjayyap/Stepsis-Atlas-by-Shortsighted-Visionaries/chunks" 
    
    # 3. Your specific summarizes output directory
    OUTPUT_DIRECTORY = "/Users/hongjayyap/Stepsis-Atlas-by-Shortsighted-Visionaries/summarizes"
